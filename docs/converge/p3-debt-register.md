...
Current baseline: latest local C7.2 recovery/report-proof takeover on `main`.
C7.2 keeps Converge-owned recovery, delivery reservation, report proof, and
reported completion inside Converge workflow state. C7.3 remains plan-only
legacy route retirement work; live slash routing, Gateway restart, shadow
routing, deploy/apply/install, legacy deletion, external action, push, PR, and
