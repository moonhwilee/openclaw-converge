...
def valid_command_record(command: str) -> dict:
    return {
        "fresh_route_context": True,
        "owner_session_key": OWNER_SESSION,
        "visible_delivery": VISIBLE,
        "state_root": "/tmp/converge-phase6-route-evidence",
        "workflow_id": f"phase6-{command.strip('/')}-workflow",
        "route_owner": "converge",
        "legacy_handler_invoked": False,
        "report_proof_ref": f"report-proof:{command}",
        "complete_reported_ref": f"complete-reported:{command}",
...
def valid_evidence() -> dict:
    return {
        ...
        "commands": {
            "/goal": valid_command_record("/goal"),
            "/verify": valid_command_record("/verify"),
            "/conv": valid_command_record("/conv"),
        },
        "aliases": {
            "/converge": {
                "promoted": False,
